...
pub fn public_suggestion(db: &rusqlite::Connection, shot: &Shot) -> Result<Value> {
    ...
    Ok(json!({"baseRevision":suggestion.base_revision,"stale":suggestion.base_revision!=shot.revision,
...
pub fn dismiss(db: &rusqlite::Connection, id: &str) -> Result<()> {
    db.execute("DELETE FROM records WHERE kind='analysis_suggestion' AND id=?1", [id])?;
    Ok(())
}
...
pub fn accept(db: &rusqlite::Connection, shot: &mut Shot, fields: &[String]) -> Result<()> {
    ...
    ensure!(suggestion.base_revision == shot.revision, "建议已过期，请重新分析当前片段");
    ensure!(!fields.is_empty() && fields.iter().all(|f| ["name", "description", "details", "tags", "holidays", "unsupportedClaims"].contains(&f.as_str())), "请选择有效的建议项");
    let s = suggestion.shot;
    ensure!(s.start_ticks == shot.start_ticks && s.end_ticks == shot.end_ticks, "建议范围已过期，请重新分析当前片段");
    for field in fields {
        match field.as_str() {
            ...
            "tags" => {
                ...
                shot.tag_evidence.retain(|e| shot.holiday_tags.contains(&e.tag));
                crate::tag_evidence::merge(&mut shot.tag_evidence, s.tag_evidence.iter().filter(|e| s.tags.contains(&e.tag)).cloned());
            ...
            "holidays" => {
                ...
                crate::tag_evidence::merge(&mut shot.tag_evidence, s.tag_evidence.iter().filter(|e| s.holiday_tags.contains(&e.tag)).cloned());
            }
    ...
    shot.status = if shot.direct_upload { "tag_review" } else if shot.output_path.is_some() { "review" } else { "draft" }.into();
