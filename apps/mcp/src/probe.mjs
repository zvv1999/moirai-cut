 ...
 *
 * Requires a Chrome on the debugging port with an OpenCut editor tab open.
 */
...
check(
  ...
  ["status", "get_state", "list_operations", "apply_operation", "render_frames", "undo", "redo"].every(
...
const TRANSIENT = new Set(["bridge_missing", "target_gone", "no_editor_tab", "driver_error"]);
...
check("status reaches a live editor tab", status.payload.connected, status.payload.tab?.url);
...
check(
  ...
  applied.payload.applied === true && applied.payload.revision === baseRevision + 1,
...
check(
  ...
  replay.payload.deduplicated === true && replay.payload.revision === applied.payload.revision,
...
const callGuard = async () => {
  for (let attempt = 0; attempt < 12; attempt += 1) {
    const response = await callBridge({
      ...
      value: { ok: false, error: { code: "driver_error", message: String(error?.message ?? error) } },
    }));
    if (!TRANSIENT.has(response.value?.error?.code)) return response;
    await new Promise((resolve) => setTimeout(resolve, 1500));
  }
  return { value: { ok: false, error: { code: "driver_error", message: "bridge never came back" } } };
};
...
if (clip) {
  const moved = await call("apply_operation", {
    operation: {
      type: "element.move",
      moves: [{ trackId: clip.trackId, elementId: clip.id, startTimeSeconds: 4.5 }],
    },
  ...
  const mispaired = await call("apply_operation", {
    operation: {
      type: "element.delete",
      elements: [{ trackId: "00000000-0000-4000-8000-000000000000", elementId: clip.id }],
    },
  ...
  const ours = live.payload.tracks.flatMap((t) =>
    t.elements.filter((e) => e.name.startsWith(titleName)).map((e) => ({ trackId: t.id, elementId: e.id })),
  );
  ...
  check(
    ...
    cleaned.payload.applied === true &&
      !finalState.payload.tracks.flatMap((t) => t.elements).some((e) => e.name.startsWith(titleName)),
    `deleted ${ours.length}`,
  );
}
...
const nowMuted = preRestore.payload.tracks?.find((t) => t.id === track.id)?.muted;
...
check(
  ...
  Boolean(restored) &&
    ...
    !after.payload.tracks.flatMap((t) => t.elements).some((e) => e.name.startsWith(titleName)),
