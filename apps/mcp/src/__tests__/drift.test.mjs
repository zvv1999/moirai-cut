...
function registryOperationTypes() {
	...
	assert.ok(
		types.length > 0,
		"parsed zero operation types — the parser is broken, not the code",
	);
	return types;
}
...
test("the MCP schema advertises exactly what the page registry can execute", () => {
	assert.deepEqual(
		SCHEMA_OPERATION_TYPES.slice().sort(),
		registryOperationTypes().sort(),
	);
});

test("SCHEMA_OPERATION_TYPES matches the zod union it claims to describe", () => {
	const inUnion = OperationSchema.options.map(
...
test("a malformed operation is refused by the schema, not sent to the page", () => {
	assert.equal(
		OperationSchema.safeParse({ type: "track.toggleMute" }).success,
		false,
	);
	assert.equal(
		OperationSchema.safeParse({ type: "nope", trackId: "t" }).success,
		false,
	);
	assert.equal(
		OperationSchema.safeParse({ type: "track.toggleMute", trackId: "t" })
			.success,
		true,
	);
});
...
test("FCPXML handoff is revisioned and non-destructive", () => {
	const server = createOpenCutMcpServer();
	const exportFcpxml = server._registeredTools?.export_fcpxml;

	assert.equal(exportFcpxml?.annotations?.readOnlyHint, false);
	assert.equal(exportFcpxml?.annotations?.destructiveHint, false);
});
