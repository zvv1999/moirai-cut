...
async function listTargets(port) {
	...
	if (!response.ok)
		throw new CdpError(`DevTools endpoint returned ${response.status}.`);
...
export async function findEditorTarget({
	...
	const pages = targets.filter(
		(t) =>
			t.type === "page" &&
			typeof t.url === "string" &&
			t.url.includes(DEFAULT_URL_MATCH),
	);
	const matches = projectId
		? pages.filter((t) => t.url.includes(projectId))
		: pages;
	...
	const [match] = matches;
	if (
		typeof match.webSocketDebuggerUrl !== "string" ||
		!match.webSocketDebuggerUrl
	) {
...
export function evaluateInPage({
	...
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(() => {
				...
				reject,
				new CdpError(`Timed out after ${timeoutMs}ms evaluating in the page.`, {
					code: "timeout",
				}),
			);
			try {
		...
		socket.on("error", (error) =>
				...
				reject,
				new CdpError(`CDP socket error: ${error.message}`, {
					code: "socket_error",
				}),
			),
		);
		...
		socket.on("close", (code) =>
				...
				reject,
				new CdpError(
					`The editor tab closed the debugger connection (code ${code}).`,
					{
						code: "target_gone",
		...
		socket.on("message", (raw) => {
			...
			if (message.error) {
				return done(
					reject,
					new CdpError(message.error.message || "CDP call failed"),
				);
			}
	...
	if (response.value?.ok !== true) {
		throw new CdpError(
			response.value?.error?.message ??
				"Bridge answered but not with an ok envelope.",
...
async function waitUntilDrivable({ port, projectId, deadline }) {
	...
	throw new CdpError(
		`The editor for ${projectId} never became drivable in time.`,
		{
			...
			hint: String(lastError?.message ?? lastError ?? ""),
		},
	);
}
...
async function doOpenEditorTab({
	port,
	projectId,
	base,
	timeoutMs,
	...
	if (!created?.id || !created?.webSocketDebuggerUrl) {
		throw new CdpError("Chrome created no usable tab.", {
...
export async function closeEditorTab({ port = DEFAULT_PORT, projectId } = {}) {
	...
	if (!response.ok) {
		throw new CdpError(`Closing the tab failed: ${response.status}.`, {
			code: "close_failed",
		});
	}
