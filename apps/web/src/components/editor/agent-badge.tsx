...
export function AgentBadge() {
	...
	const createSnapshot = async () => {
		...
		if (!response.ok) {
			toast.error(errorMessage({ value: payload, fallback: "保存快照失败" }));
			return;
		}
	...
	const compare = async (revision: number) => {
		...
		try {
			...
			if (!response.ok || !isProjectRevisionDiff(payload)) {
				toast.error(errorMessage({ value: payload, fallback: "版本比较失败" }));
				return;
			}
			setDiff(payload);
		} finally {
	...
	const restore = async (revision: number) => {
		...
		const payload: unknown = await response.json().catch(() => ({}));
		if (response.ok) {
			toast(`快照已恢复为版本 ${revisionOf(payload) ?? "未知"}`);
			setDiff(null);
			setConfirmRevision(null);
			await refreshHistory();
		} else {
		...
		<>
			{openSurface === "smart-edit" ? (
				<aside
					...
					data-testid="agent-workbench-sidebar"
					aria-label="智能剪辑侧栏"
					className="h-full w-[clamp(320px,28vw,440px)] max-w-[46%] shrink-0 overflow-hidden border-r border-white/8 bg-[#111315] text-slate-100"
				>
					<AgentWorkbench onClose={() => setOpenSurface(null)} />
				</aside>
			) : null}
			...
			<div className="fixed right-4 bottom-4 z-50 flex flex-col items-end gap-2">
										...
										{diff.changes.length === 0 ? (
											...
											<ul className="mb-2 max-h-24 space-y-1 overflow-y-auto text-[10px]">
												{diff.changes.slice(0, 12).map((change, index) => (
													<li
														key={`${change.kind}-${change.elementId}-${index}`}
													>
				...
				<div className="border-border bg-background/85 flex items-center gap-1 rounded-full border p-1 shadow-lg backdrop-blur-md">
					...
					<button
						...
						aria-expanded={openSurface === "smart-edit"}
						aria-controls="agent-workbench-sidebar"
						onClick={() =>
