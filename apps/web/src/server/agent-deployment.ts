...
export async function installOpenCutMcp({
	...
	if (await inspectOpenCutMcpRegistration({ provider, binary, run })) {
		return {
			...
			changed: false,
			verified: true,
			message: `${providerLabel(provider)} 已安装 Moirai Cut MCP。`,
		};
	}
	...
	if (!verified) {
		throw new Error(
			`${providerLabel(provider)} 已写入配置，但 Moirai Cut MCP 验证失败。`,
		);
	}
	return {
		provider,
		changed: true,
		verified: true,
		message: `${providerLabel(provider)} 已安装并验证 Moirai Cut MCP。`,
	};
}
