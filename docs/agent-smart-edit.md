# Moirai Cut × Codex 智能剪辑工作流

本文是 Moirai Cut `agent-drivable` 工程的 Agent 操作规范。目标是让编辑器内的
“智能剪辑”和 Codex App 中的工程会话共享同一份上下文、素材理解结果和工程文件，
并允许用户与 Agent 反复交替编辑。

## 0. 共享编辑循环

Moirai Cut 不把 Agent 输出当作一次性成片，而是把三种交互放进同一个工程状态：

1. 用户通过对话提出复杂意图，并引用素材、元素或时间段；
2. Agent 读取最新 revision，提出或执行修改；
3. 变化落到当前可编辑时间线，过程和结果对用户可见；
4. 用户直接拖动、裁切、调整参数并实时预览；
5. Agent 从人工修改后的同一工程继续下一轮。

Agent 的编辑必须可回读、可人工修改、可撤销、可验证。不得为了续聊生成一个脱离当前
工程的新时间线，也不得把“已生成结果”伪装成无法检查的黑盒输出。

## 1. 双向链路

```mermaid
flowchart LR
  UI["Moirai Cut 编辑器"] -->|"选择元素 / 时间段 / 素材"| Context["Agent Context v1"]
  Context --> Presence["编辑器 Presence API"]
  Context --> Chat["智能剪辑 SSE 会话"]
  Chat --> Host["共享 Codex app-server"]
  App["Codex App"] --> Host
  Host --> Codex["同一 thread 与事件通道"]
  Codex --> MCP["Moirai Cut MCP"]
...
1. **Moirai Cut 内置智能剪辑**：编辑器把当前引用上下文随消息发送到
...
`fetch`。这份配置只注册 Moirai Cut，不依赖 LocalCut。
...
Moirai Cut MCP；工作区配置作为用户从非共享入口启动 Codex 时的兼容兜底。
...
这不是把 Codex App 窗口嵌入网页。浏览器只实现轻量展示和 Moirai Cut 引用交互，
认证、任务历史、续聊、模型执行与流式事件均使用 Codex 原生 App Server 协议。
Moirai Cut 和 Codex App 必须连接同一个 loopback WebSocket 宿主；默认地址为：

```bash
OPENCUT_CODEX_APP_SERVER_URL=ws://127.0.0.1:48721
```

Moirai Cut 在首次请求能力或发起会话时检查 `/readyz`，宿主不存在时以当前 Codex
运行时自动启动一次；同一 Moirai Cut 服务进程内的所有工具档位、API 路由和工程会话
...
桌面 App，然后在 Moirai Cut 仓库执行：

```bash
bun run codex:shared-app
```

启动器先触发 Moirai Cut 建立共享宿主，再以
...
改变宿主。自定义端口时，Moirai Cut 环境变量和桌面 App 启动环境必须使用同一个 URL。

### Codex App 工作区与任务可见性

Moirai Cut 智能剪辑创建、恢复或迁移 task 时使用 `OPENCUT_CODEX_WORKSPACE_ROOT` 作为
...
`opencut-projects` 工程文件目录，Moirai Cut MCP 的 `cwd` 仍是 `opencut-classic`。
...
运行时根，不接受 Codex 桌面端项目的 `projectId`。Moirai Cut 不修改
...
Moirai Cut 内置智能剪辑已经随消息携带上下文，Codex App 则通过 Presence API 获取。
...
| Codex App 续聊缺少 Moirai Cut    | 检查 task 的 `cwd` 所在工作区 `.codex/config.toml` 是否包含 `mcp_servers.opencut`。          |
