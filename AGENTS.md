...
调色以“原色”或“AI 调色”为基础，自定义参数始终可用并叠加在基础调色之后。自定义调色支持亮度、对比度、饱和度、曝光 EV、色温、色调、高光、阴影、白色色阶、黑色色阶和自然饱和度；重置仅恢复自定义参数，不切换基础模式。Rust 合成同一 LUT 供实时预览和最终加工使用。
...
低成本跨电脑模式下，库数据库与库内素材缓存直接位于目录的 `.moirai-library/data`，NAS 或本机目录是唯一数据源；首次迁移会把旧本机库复制到该目录。各电脑仍保留运行配置和模型凭据，不再为同一库建立独立数据库。SQLite 使用共享忙等待，建议轮流操作，不要同时启动多个后台任务或并行保存。
